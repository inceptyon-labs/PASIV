...
Add a completion summary comment to an issue (for context sharing with sibling Tasks).
...
Arguments: owner, repo, parent_issue_number

```bash
# Get all sub-issues of the parent
SIBLINGS=$(gh api graphql -f query="
query {
  repository(owner: \"$OWNER\", name: \"$REPO\") {
...
Return: List of completion summaries from closed sibling Tasks.
