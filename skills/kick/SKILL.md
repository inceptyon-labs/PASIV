...
Each step-skill shares this session's context (`IDENTIFIER`, `WORKFLOW_*`, `REVIEW_TIER`, `PARENT_IDENTIFIER`, …) — set those here, then invoke the skills in order, waiting for each one's `>>> … COMPLETE <<<` marker before the next.
...
1. **Skill:** `plan` — produces the plan, sets `REVIEW_TIER`, creates native tasks. Wait for `>>> PLAN COMPLETE <<<`.
...
3. **Skill:** `review` — runs `REVIEW_TIER` (skips itself if `WORKFLOW_REVIEW` false or tier SKIP). Wait for `>>> REVIEW COMPLETE <<<`.
...
Display the hierarchy with each Task's tier and the per-Task workflow. Run the baseline **once** for the whole Epic/Feature (`test-runner`; on failure ask as in Step 0.75).

- **`WORKFLOW_PLAN_APPROVAL` true** → AskUserQuestion: "Approve with these tiers?" → Yes (autonomous) / Customize tiers / Cancel. Store `REVIEW_TIERS` map.
- **false** → auto-approve with recommended tiers (all "SKIP" if `WORKFLOW_REVIEW` also false). Store `REVIEW_TIERS`.
...
For each Task: set `IDENTIFIER`/`PARENT_IDENTIFIER` and `REVIEW_TIER = REVIEW_TIERS[task]`, then run the **same step-skills** — but **skip baseline** (already run) and **skip plan's approval gate** (pre-approved):
...
2. **Skill:** `plan` — note `PLAN_PREAPPROVED=true` so it uses the given `REVIEW_TIER` and does **not** re-ask. Display the plan briefly.
3. **Skill:** `execute`.
4. **Skill:** `review` (uses `REVIEW_TIER`; skip if false/SKIP).
...
Parents auto-close via `finish`'s cascade. Find the next issue (`gh issue list --label priority:high --state open --limit 1`, fallback any open) and report the Epic/Feature summary with each Task ✓ and its tier, then "Run `/kick next` to continue."
